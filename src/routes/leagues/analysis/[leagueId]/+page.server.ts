...
export const load = (async ({ params }) => {
	...
	const rosters = teams.map((team) => {
		const players = picks.filter((pick) => String(pick.team_id) === String(team.espn_team_id)).map((pick) => {
			...
			return { id: row?.id ?? null, espnId: pick.espn_player_id, name: row?.full_name ?? pick.player_name,
				...
				pickNumber: pick.pick_number, round: pick.round_number };
		...
		return { ...team, players, starters, bench, counts: countPositions(players), rawScore: starterValue + depthValue };
	});
	...
	const tradeTargets = powerRankings.filter((team) => !team.is_user).flatMap((team) => team.players
		.filter((player) => needs.includes(player.position) && Number(team.counts[player.position] ?? 0) > Number(tradeDepth[player.position] ?? 99))
		.map((player) => ({ ...player, fromTeam: team.team_name, ownerName: team.owner_name,
	...
	return { league: { id: league.id, name: league.name, seasonYear: league.season_year, teamCount: league.team_count },
		powerRankings: powerRankings.map(({ rawScore: _raw, ...team }) => team), user, positionComparison, tradeTargets,
		methodology: 'Draft-value baseline using current consensus rank, likely starters, and shallow bench depth. It is not yet a weekly projection.' };
}) satisfies PageServerLoad;

function chooseStarters(players: any[]) {
	const remaining = players.slice().sort(byRank);
	const starters: any[] = [];
