...
<main class="min-h-screen bg-slate-50 px-4 py-8 text-slate-950 sm:px-8">
	<div class="mx-auto max-w-7xl space-y-8">
		<header class="flex flex-wrap items-end justify-between gap-4">
			...
			<a href="/leagues" class="rounded-xl border border-slate-300 bg-white px-4 py-2 font-semibold shadow-sm">Back to leagues</a>
		</header>

		{#if data.user}
			<section class="grid gap-4 md:grid-cols-3">
				<div class="rounded-2xl bg-slate-950 p-6 text-white"><p class="text-sm uppercase tracking-wider text-slate-300">Your preseason power rank</p><p class="mt-2 text-5xl font-black">#{data.user.powerRank}</p><p class="mt-2 text-slate-300">of {data.league.teamCount} · score {data.user.score}</p></div>
		...
		<section class="rounded-2xl border bg-white p-6 shadow-sm"><h2 class="text-2xl font-black">Power rankings</h2><p class="mt-1 text-sm text-slate-500">Starter-weighted consensus value with shallow bench depth.</p><div class="mt-5 overflow-x-auto"><table class="w-full text-left"><thead class="border-b text-xs uppercase text-slate-500"><tr><th class="p-3">Rank</th><th class="p-3">Team</th><th class="p-3">Score</th><th class="p-3">Drafted core</th></tr></thead><tbody>{#each data.powerRankings as team}<tr class="border-b last:border-0" class:bg-blue-50={team.is_user}><td class="p-3 text-xl font-black">#{team.powerRank}</td><td class="p-3"><p class="font-bold">{team.team_name}</p>{#if team.is_user}<p class="text-xs font-bold text-blue-600">YOUR TEAM</p>{/if}</td><td class="p-3 font-bold">{team.score}</td><td class="p-3 text-sm text-slate-600">{team.starters.slice(0, 5).map((player) => player.name).join(' · ')}</td></tr>{/each}</tbody></table></div></section>

		<div class="grid gap-6 lg:grid-cols-2">
			<section class="rounded-2xl border bg-white p-6 shadow-sm"><h2 class="text-2xl font-black">Likely starting lineup</h2><p class="mt-1 text-sm text-amber-700">Preseason baseline only; weekly matchup projections come next.</p><div class="mt-4 space-y-2">{#each data.user?.starters ?? [] as player}<div class="flex items-center justify-between rounded-xl bg-slate-100 px-4 py-3"><div><span class="mr-3 inline-block w-10 font-black text-blue-600">{player.position}</span><span class="font-bold">{player.name}</span></div><span class="text-sm text-slate-500">ECR {player.rank ?? '—'}{player.byeWeek ? ` · Bye ${player.byeWeek}` : ''}</span></div>{/each}</div></section>
